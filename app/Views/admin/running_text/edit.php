...
<div class="bg-white rounded-xl shadow-sm border border-gray-100">
    <form action="<?= base_url('admin/running-text/update/'.$text['running_text_id']) ?>" method="POST" class="p-6 space-y-6">
        <?= csrf_field() ?>
        
        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Teks</label>
            <textarea name="teks" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500" required><?= old('teks', $text['teks']) ?></textarea>
        ...
        <div>
            ...
            <input type="url" name="tautan" value="<?= old('tautan', $text['tautan'] ?? '') ?>" placeholder="Contoh: https://google.com atau http://..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">
                    <option value="Aktif" <?= old('status', $text['status']) == 'Aktif' ? 'selected' : '' ?>>Aktif</option>
                    <option value="Tidak Aktif" <?= old('status', $text['status']) == 'Tidak Aktif' ? 'selected' : '' ?>>Tidak Aktif</option>
                </select>
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Urutan Tampil (Opsional)</label>
                <input type="number" name="urutan" value="<?= old('urutan', $text['urutan']) ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">
            </div>
        </div>
