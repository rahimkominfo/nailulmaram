...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden max-w-3xl">
    <div class="p-6 border-b border-gray-100">
        <h3 class="text-lg font-bold text-gray-800">Edit Video Dokumentasi</h3>
    </div>

    <form action="<?= base_url('admin/video-dokumentasi/update/' . $video['video_dokumentasi_id']) ?>" method="POST" class="p-6 space-y-6">
        <?= csrf_field() ?>
        ...
        <div>
            ...
            <input type="url" name="url_youtube" value="<?= old('url_youtube', $video['url_youtube']) ?>" required
                class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-green-500 focus:ring-2 focus:ring-green-200 outline-none transition" 
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="urutan" value="<?= old('urutan', $video['urutan']) ?>" min="0"
                    class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-green-500 focus:ring-2 focus:ring-green-200 outline-none transition">
        ...
        <div class="flex items-center justify-end space-x-3 pt-4 border-t border-gray-100">
            ...
            <button type="submit" class="px-6 py-3 bg-green-600 text-white font-bold rounded-xl hover:bg-green-700 transition">Update Video</button>
        </div>
    </form>
</div>
