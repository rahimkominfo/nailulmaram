            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div class="lg:col-span-2 space-y-6">
                    <div>
                        ...
                        <input class="w-full px-6 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:bg-white transition duration-300 font-bold text-lg" id="judul" name="judul" type="text" placeholder="Masukkan judul berita yang menarik" value="<?= old('judul') ?>" required>
                    </div>

                    <div>
                        <label class="block text-gray-700 text-xs font-black mb-2 uppercase tracking-widest">Konten Berita</label>
                        <textarea id="editor" class="w-full" name="konten" placeholder="Tuliskan isi berita di sini..."><?= old('konten') ?></textarea>
                    </div>
                </div>
                    ...
                    <div class="bg-gray-50 p-8 rounded-3xl border border-gray-100">
                        ...
                        <div class="mb-6">
                            ...
                            <select class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 transition font-bold text-sm" name="kategori_id" id="kategori_id" required>
                                ...
                                <?php foreach($kategori as $k): ?>
                                    <option value="<?= $k['kategori_id'] ?>" <?= old('kategori_id') == $k['kategori_id'] ? 'selected' : '' ?>><?= strtoupper($k['nama']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <select class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 transition font-bold text-sm" name="status" id="status" required>
                                <option value="Draf" <?= old('status') == 'Draf' ? 'selected' : '' ?>>DRAF (SIMPAN)</option>
                                <option value="Ditayangkan" <?= old('status') == 'Ditayangkan' ? 'selected' : '' ?>>TAYANGKAN (PUBLIK)</option>
                                <option value="Diarsipkan" <?= old('status') == 'Diarsipkan' ? 'selected' : '' ?>>ARSIPKAN</option>
                            </select>
                        </div>
                        ...
                        <button type="submit" class="w-full bg-green-600 hover:bg-green-700 text-white font-black py-4 rounded-2xl shadow-lg shadow-green-200 transition duration-300 uppercase tracking-widest mt-4">
                            Terbitkan Berita
                        </button>
                    </div>
                </div>
