...
<?= $this->section('content') ?>
    ...
    <div class="bg-white p-10 rounded-3xl shadow-sm border border-gray-100 max-w-4xl mx-auto">
        <h3 class="text-2xl font-black text-gray-800 tracking-tighter uppercase mb-8 border-b-4 border-blue-600 w-fit pb-2">Edit Flayer</h3>
        
        <form action="<?= base_url('admin/flayer/update/'.$flayer['flayer_id']) ?>" method="post">
            <?= csrf_field() ?>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-6">
                    <div>
                        <label class="block text-gray-700 text-xs font-black mb-2 uppercase tracking-widest" for="judul">Judul Flayer</label>
                        <input class="w-full px-6 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition duration-300 font-bold" id="judul" name="judul" type="text" value="<?= old('judul', $flayer['judul']) ?>" required>
                    </div>

                    <div>
                        <label class="block text-gray-700 text-xs font-black mb-2 uppercase tracking-widest" for="label">Label (Opsional)</label>
                        <input class="w-full px-6 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition duration-300 font-bold" id="label" name="label" type="text" value="<?= old('label', $flayer['label']) ?>">
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-gray-700 text-[10px] font-black mb-2 uppercase tracking-widest" for="urutan">Urutan Tampil</label>
                            <input class="w-full px-6 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition duration-300 font-bold" id="urutan" name="urutan" type="number" value="<?= old('urutan', $flayer['urutan']) ?>" required>
                        </div>
                        <div>
                            ...
                            <select class="w-full px-6 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition duration-300 font-bold" id="status" name="status" required>
                                <option value="Aktif" <?= old('status', $flayer['status']) == 'Aktif' ? 'selected' : '' ?>>AKTIF</option>
                                <option value="Tidak Aktif" <?= old('status', $flayer['status']) == 'Tidak Aktif' ? 'selected' : '' ?>>TIDAK AKTIF</option>
                            </select>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function updatePreview(url) {
            ...
            if (url) {
                ...
                preview.src = 'https://placehold.co/600x400?text=No+URL';
            }
        }
    </script>
<?= $this->endSection() ?>
