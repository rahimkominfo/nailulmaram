            ...
            <div class="absolute inset-0 flex flex-col justify-center items-center text-center text-white px-6">
                <h1 class="text-3xl md:text-6xl font-black mb-4 md:mb-6 drop-shadow-2xl tracking-tighter uppercase">
                    Selamat Datang di <br> <span
                        class="text-green-400"><?= $profil['nama_masjid'] ?? 'Masjid Jami' ?></span>
                </h1>
                ...
                <p class="text-lg md:text-3xl max-w-4xl font-light italic text-gray-100 drop-shadow-md leading-relaxed">
                    "Hanyalah yang memakmurkan masjid-masjid Allah ialah orang-orang yang beriman kepada Allah dan hari
                    kemudian."
                </p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-3xl shadow-2xl p-8 border border-gray-100">
        <div class="flex flex-col md:flex-row justify-between items-center mb-8">
            <div class="text-center md:text-left">
                ...
                <p class="text-green-600 font-semibold mt-1"><?= format_indo(date('Y-m-d')) ?> | <span
                        class="text-orange-600"><?= $hijriah ?></span></p>
            </div>
            <div
                class="mt-4 md:mt-0 italic bg-gray-50 px-4 py-2 rounded-xl text-sm border border-gray-100 md:self-start">
        ...
        <div id="countdown-container"
            class="hidden mb-8 bg-gradient-to-r from-green-600 to-green-800 rounded-3xl p-6 text-white shadow-lg flex flex-col md:flex-row items-center justify-between">
        ...
        <?php if ($jadwal): ?>
            <div class="grid grid-cols-2 md:grid-cols-6 gap-4" id="prayer-times-grid">
                <div data-prayer="Subuh"
                    class="bg-green-50 p-6 rounded-2xl text-center border-b-4 border-green-600 shadow-sm transition hover:scale-105 duration-300">
                    <p class="text-xs font-bold text-green-600 uppercase mb-2">Subuh</p>
                    <p class="text-3xl font-black text-green-900"><?= $jadwal['subuh'] ?></p>
                </div>
                <div data-prayer="Dzuhur"
                    class="bg-green-50 p-6 rounded-2xl text-center border-b-4 border-green-600 shadow-sm transition hover:scale-105 duration-300">
                ...
                <div data-prayer="Isya"
                    class="bg-green-50 p-6 rounded-2xl text-center border-b-4 border-green-600 shadow-sm transition hover:scale-105 duration-300">
                    <p class="text-xs font-bold text-green-600 uppercase mb-2">Isya</p>
                    <p class="text-3xl font-black text-green-900"><?= $jadwal['isya'] ?></p>
                </div>
                <div data-prayer="Imsak"
                    class="bg-gray-100 p-6 rounded-2xl text-center border-b-4 border-gray-500 shadow-sm transition hover:scale-105 duration-300">
...
<section id="berita" class="py-24 container mx-auto px-6">
    ...
    <div
        class="relative w-full h-[550px] md:h-[400px] rounded-3xl overflow-hidden shadow-xl border border-gray-100 bg-white group">
        <?php if (empty($berita)): ?>
            <div class="flex items-center justify-center h-full text-gray-400 italic">Belum ada berita yang ditayangkan.
            </div>
        <?php else: ?>
            <?php foreach ($berita as $idx => $b): ?>
                <div
                    class="news-slide absolute inset-0 transition-all duration-700 ease-in-out <?= $idx === 0 ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none' ?>">
                    <div class="flex flex-col md:flex-row h-full">
                        ...
                        <div class="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center bg-white">
                            ...
                            <a href="<?= base_url('berita/baca/' . $b['slug']) ?>"
                                class="inline-flex items-center text-green-600 font-bold hover:text-green-800 transition group/link">
                                Baca Selengkapnya <i
                                    class="fas fa-arrow-right ml-2 transform group-hover/link:translate-x-1 transition"></i>
                            </a>
                        </div>
                    </div>
                </div>
    ...
    <div class="mt-12 text-center">
        <a href="<?= base_url('berita') ?>"
            class="inline-flex items-center px-8 py-3 border-2 border-green-600 text-green-600 font-bold rounded-full hover:bg-green-600 hover:text-white transition-all duration-300 group shadow-sm hover:shadow-md">
            Lihat Semua Berita
    ...
    <div class="container mx-auto px-6">
        ...
        <div class="relative">
            <div id="flayer-slider-container" class="overflow-hidden">
                <div id="flayer-slider"
                    class="flex transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)]">
                    <?php if (empty($flayers)): ?>
                        <div class="flex-none w-full text-center py-12 text-gray-400 italic">Belum ada flayer yang aktif.
                        </div>
                    <?php else: ?>
                        <?php foreach ($flayers as $f): ?>
                            ...
                            <div class="flex-none w-full md:w-1/3 px-4">
                                <div
                                    class="group relative overflow-hidden shadow-2xl aspect-[3/4] bg-white border-4 border-white">
                                    <img src="<?= (strpos($f['gambar_url'], 'http') === 0) ? $f['gambar_url'] : base_url('uploads/flayer/' . $f['gambar_url']) ?>"
                                        alt="<?= $f['judul'] ?>"
                                        class="w-full h-full object-contain transform group-hover:scale-110 transition duration-1000">
                                    <div
                                        class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-700 flex flex-col justify-end p-8">
                                        <?php if ($f['label']): ?>
                                            <span
                                                class="bg-orange-600 text-[10px] w-fit px-3 py-1 rounded-full text-white font-bold mb-3 uppercase tracking-widest"><?= $f['label'] ?></span>
                                        <?php endif; ?>
                                        <p class="text-white font-black text-2xl uppercase tracking-tighter leading-tight">
                                            <?= $f['judul'] ?></p>
                                    </div>
                                </div>
                            </div>
    ...
    <div class="container mx-auto px-6">
        ...
        <div class="mt-16 text-center animate-fade-in-up delay-300">
            <a href="<?= base_url('galeri') ?>"
                class="inline-flex items-center px-8 py-3 border-2 border-green-500 text-green-400 font-bold rounded-full hover:bg-green-500 hover:text-gray-950 transition-all duration-300 group shadow-[0_0_15px_rgba(34,197,94,0.3)] hover:shadow-[0_0_25px_rgba(34,197,94,0.6)]">
                Lihat Semua Galeri
                <i class="fas fa-arrow-right ml-2 transform group-hover:translate-x-1 transition-transform"></i>
            </a>
        </div>
...
<section id="video-youtube" class="py-12 container mx-auto px-6 overflow-hidden">
    <div class="bg-white rounded-[40px] shadow-2xl p-4 md:p-8 border border-gray-100">
        ...
        <div class="relative">
            <div id="video-slider-container" class="overflow-hidden -mx-4">
                <div id="video-slider"
                    class="flex transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)]">
                    <?php if (empty($videos)): ?>
                        <p class="text-gray-500 italic flex-none w-full text-center py-8">Belum ada video dokumentasi.</p>
                    <?php else: ?>
                        <?php foreach ($videos as $video): ?>
                            <?php
                            // Extract video ID from youtube URL to embed properly
                            ...
                            <div class="flex-none w-full md:w-1/2 px-4">
                                <div class="flex flex-col h-full">
                                    <div
                                        class="relative w-full aspect-video rounded-3xl overflow-hidden shadow-xl border-4 border-green-50/50 group mb-3">
        ...
        <div id="video-indicators" class="flex justify-center space-x-3 mt-6">
            <!-- Indicators dynamically generated -->
        </div>
    </div>
</section>
        ...
        <div class="flex flex-col lg:flex-row">
            ...
            <div class="w-full lg:w-[40%] p-8 md:p-12 lg:p-16 flex flex-col justify-center bg-green-50/30">
                ...
                <p
                    ...
                    kita semua dalam memakmurkan rumah Allah."
                </p>
                <div class="flex flex-wrap gap-4">
                    <a href="<?= base_url('cerita-inspiratif') ?>"
                        ...
                        Mulai Membaca
                        <i
                            class="fas fa-book-open ml-3 transform group-hover/btn:translate-x-1 transition-transform"></i>
                    </a>
                </div>
            </div>
        </div>
...
<section id="ayo-baca-quran" class="py-24 container mx-auto px-6">
    <div class="bg-white rounded-[40px] shadow-2xl overflow-hidden border border-gray-100 group">
        <div class="flex flex-col lg:flex-row-reverse">
            <div
                class="w-full lg:w-[50%] relative overflow-hidden h-[400px] md:h-[600px] bg-green-50/30 flex items-center justify-center p-8">
                <img loading="lazy" src="<?= base_url('images/ayo_baca_quran.webp') ?>" alt="Ayo Baca Quran"
            ...
            <div class="w-full lg:w-[50%] p-8 md:p-12 lg:p-16 flex flex-col justify-center bg-white">
                ...
                <div class="flex flex-wrap gap-4">
                    <a href="<?= base_url('quran') ?>"
                        ...
                        Baca Quran
                        <i
                            class="fas fa-arrow-right ml-3 transform group-hover/btn:translate-x-1 transition-transform"></i>
                    </a>
                </div>
            </div>
        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const indicatorContainer = document.getElementById('video-indicators');

        if (!slider) return;

        const slides = slider.children;
        if (slides.length === 0 || (slides.length === 1 && slides[0].tagName === 'P')) {
            // No valid slides
        ...
        function updateSlider() {
            ...
            dots.forEach((dot, idx) => {
                if (idx === currentIdx) {
                    dot.className = 'w-3 h-3 rounded-full transition-all duration-300 bg-green-600 ring-4 ring-green-200';
                } else {
                    dot.className = 'w-3 h-3 rounded-full transition-all duration-300 bg-gray-300 hover:bg-green-400';
                }
            });
        }
