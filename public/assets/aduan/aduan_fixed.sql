...
INSERT INTO `pengurus` (`nama`, `foto`, `jabatan`, `jenis`, `bidang`, `sub_bidang`, `ikon`) VALUES
...
CREATE TABLE IF NOT EXISTS `aduan` (
  `aduan_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `kode_tiket` VARCHAR(50) NOT NULL COMMENT 'ID unik untuk tracking',
  `nama_pengirim` VARCHAR(150) DEFAULT NULL COMMENT 'Nama jamaah',
  `kontak_pengirim` VARCHAR(100) DEFAULT NULL COMMENT 'WhatsApp atau Email',
  `pengurus_id` BIGINT UNSIGNED NOT NULL COMMENT 'Relasi ke tabel pengurus',
  `judul_aduan` VARCHAR(255) NOT NULL COMMENT 'Judul aduan',
  `isi_aduan` TEXT NOT NULL COMMENT 'Isi lengkap aduan',
  `lampiran_file` VARCHAR(255) DEFAULT NULL COMMENT 'Path file foto/dokumen',
  `status_aduan` ENUM('Menunggu', 'Diproses', 'Diteruskan', 'Selesai', 'Ditolak') NOT NULL DEFAULT 'Menunggu',
  `tanggapan_pengurus` TEXT DEFAULT NULL COMMENT 'Jawaban atau solusi dari pengurus',
  `waktu_dibuat` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `waktu_diperbarui` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
